...
namespace WeaponPaints
{
	public partial class WeaponPaints
	{
		...
		public HookResult OnItemPickup(EventItemPickup @event, GameEventInfo _)
		{
			...
			// Check if this is a knife from another player (defindex mismatch = different knife type)
			if (HasChangedKnife(player, out var knifeName))
			{
				var holderKnifeDefindex = WeaponDefindex.FirstOrDefault(x => x.Value == knifeName).Key;
				if (holderKnifeDefindex != 0 && holderKnifeDefindex != weaponDefIndex)
					return HookResult.Continue; // Different knife type, leave untouched
			}
			
			// Mark slot as picking up a knife so GivePlayerWeaponSkin skips override
			_pickingUpKnife.Add(player.Slot);
			try
			{
			...
			{
				_pickingUpKnife.Remove(player.Slot);
			}
